...
class ECPayLogistics
{
    ...
    public function CreateShippingOrder($ButtonDesc = '物流訂單建立', $Target = '_self')
    {
        ...
        if ($this->PostParams['LogisticsSubType'] == LogisticsSubType::UNIMART ||
        ...
        if ($this->PostParams['GoodsAmount'] < $MinAmount || $this->PostParams['GoodsAmount'] > $MaxAmount) {
        ...
        if ($this->PostParams['IsCollection'] == IsCollection::NO) {
            ...
            if ($this->PostParams['CollectionAmount'] < $MinAmount || $this->PostParams['CollectionAmount'] > $MaxAmount) {
                throw new Exception('Invalid CollectionAmount.');
            }
        }
        
        if ($this->PostParams['LogisticsSubType'] == LogisticsSubType::HILIFE_C2C || $this->PostParams['LogisticsSubType'] == LogisticsSubType::UNIMART_C2C) {
        ...
        if ($this->PostParams['LogisticsType'] == LogisticsType::HOME) {
            ...
            if (empty($this->PostParams['SenderPhone']) && empty($this->PostParams['SenderCellPhone'])) {
                throw new Exception('SenderPhone or SenderCellPhone is required when LogisticsType is Home.');
            }
        } elseif ($this->PostParams['LogisticsSubType'] == LogisticsSubType::HILIFE_C2C || $this->PostParams['LogisticsSubType'] == LogisticsSubType::UNIMART_C2C) {
        ...
        if ($this->PostParams['LogisticsType'] == LogisticsType::HOME) {
            ...
            if (empty($this->PostParams['ReceiverPhone']) && empty($this->PostParams['ReceiverCellPhone'])) {
        ...
        if ($this->PostParams['LogisticsSubType'] == LogisticsSubType::ECAN && $this->PostParams['Temperature'] !== Temperature::ROOM) {
            // 物流子類型為宅配通(ECAN)時，溫層(Temperature)只能用常溫(ROOM)
            throw new Exception('Temperature should be ROOM when LogisticsSubType is ECAN.');
        }

        if ($this->PostParams['LogisticsSubType'] == LogisticsSubType::ECAN && date('Ymd', strtotime($this->PostParams['ScheduledDeliveryDate'])) < date('Ymd', strtotime('+3 day'))) {
            // 指定送達日期為該訂單建立時間 + 3 天
        ...
        if ($this->PostParams['LogisticsType'] == LogisticsType::HOME && $this->PostParams['Temperature'] == Temperature::FREEZE) {
    ...
    public function CreateHomeReturnOrder()
    {
        ...
        if ($IsAllpayLogisticsIdEmpty === true && empty($this->PostParams['LogisticsSubType'])) {
    ...
    public function UpdateUnimartLogisticsInfo()
    {
        ...
        if (empty($this->PostParams['ShipmentDate']) && empty($this->PostParams['ReceiverStoreID'])) {
        ...
        if (empty($Value)) {
            ...
            if (isset($this->PostParams['LogisticsType'])) {
                ...
                if ($LogisticsType === LogisticsType::CVS) {
                    if ($Value != LogisticsSubType::FAMILY &&
                        $Value != LogisticsSubType::UNIMART &&
                        $Value != LogisticsSubType::HILIFE &&
                        $Value != LogisticsSubType::FAMILY_C2C &&
    ...
    private function ValidateIsCollection($AllowEmpty = false)
    {
        ...
        if (empty($Value)) {
            ...
            if ($this->PostParams['LogisticsType'] == LogisticsType::HOME && $Value == IsCollection::YES) {
