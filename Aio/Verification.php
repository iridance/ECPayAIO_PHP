...
abstract class Verification
{
    ...
    public function check_invoiceString($arExtend = [])
    {
        ...
        if (!array_key_exists('CarruerType', $arExtend)) {
            $arExtend['CarruerType'] = Enum\CarruerType::None;
        } else {
            //有設定統一編號的話，載具類別不可為合作特店載具或自然人憑證載具。
            $notPrint = [Enum\CarruerType::Member, Enum\CarruerType::Citizen];
        ...
        if (!array_key_exists('CustomerID', $arExtend)) {
            ...
            if ($arExtend['CarruerType'] == Enum\CarruerType::Member && strlen($arExtend['CustomerID']) == 0) {
                array_push($arErrors, "CustomerID is required.");
            }
        }
        // 捐贈註記 Donation(預設為No)
        if (!array_key_exists('Donation', $arExtend)) {
            $arExtend['Donation'] = Enum\Donation::No;
        } else {
            //若有帶統一編號，不可捐贈
            if (strlen($arExtend['CustomerIdentifier']) > 0 && $arExtend['Donation'] != Enum\Donation::No) {
        ...
        if (!array_key_exists('Print', $arExtend)) {
            $arExtend['Print'] = Enum\PrintMark::No;
        } else {
            //捐贈註記為捐贈(Yes)時，請設定不列印(No)
            if ($arExtend['Donation'] == Enum\Donation::Yes && $arExtend['Print'] != Enum\PrintMark::No) {
                array_push($arErrors, "Print should be No.");
            }
            // 統一編號不為空字串時，請設定列印(Yes)
            if (strlen($arExtend['CustomerIdentifier']) > 0 && $arExtend['Print'] != Enum\PrintMark::Yes) {
        ...
        if (!array_key_exists('CustomerName', $arExtend)) {
            ...
            if ($arExtend['Print'] == Enum\PrintMark::Yes && strlen($arExtend['CustomerName']) == 0) {
        ...
        if (!array_key_exists('CustomerAddr', $arExtend)) {
            ...
            // 列印註記為列印(Yes)時，此參數不可為空字串
            if ($arExtend['Print'] == Enum\PrintMark::Yes && strlen($arExtend['CustomerAddr']) == 0) {
        ...
        if (!array_key_exists('ClearanceMark', $arExtend)) {
            ...
            if ($arExtend['TaxType'] == Enum\TaxType::Zero && ($arExtend['ClearanceMark'] != Enum\ClearanceMark::Yes || $arExtend['ClearanceMark'] != Enum\ClearanceMark::No)) {
                array_push($arErrors, "ClearanceMark is required.");
            }
            if (strlen($arExtend['ClearanceMark']) > 0 && $arExtend['TaxType'] != Enum\TaxType::Zero) {
        ...
        if ($arExtend['Donation'] == Enum\Donation::Yes) {
        ...
        if (!array_key_exists('InvoiceItems', $arExtend)) {
            ...
            if ($arExtend['TaxType'] == Enum\TaxType::Mix) {
                if (in_array(Enum\TaxType::Dutiable, $tmpItemTaxType) and in_array(Enum\TaxType::Free, $tmpItemTaxType)) {
                    // Do nothing
                } else {
                    $tmpItemTaxType = [];
                }
