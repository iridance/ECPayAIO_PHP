...
class AllInOne
{
    ...
    //取得付款結果通知的方法
    public function CheckOutFeedback($post = null)
    {
        if (empty($post) === true) {
            $post = $_POST;
        }
        return $arFeedback = CheckOutFeedback::CheckOut(array_merge($post, ['EncryptType' => $this->EncryptType]), $this->HashKey, $this->HashIV, 0);
    }
