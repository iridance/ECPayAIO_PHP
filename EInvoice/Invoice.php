...
class Invoice
{
    ...
    public function __construct()
    {
        ...
        $this->Send = [
            ...
            "CustomerEmail"      => '',
            "ClearanceMark"      => '',
            "Print"              => Enum\PrintMark::No,
            "Donation"           => Enum\Donation::No,
            "LoveCode"           => '',
            "CarruerType"        => Enum\CarruerType::None,
            "CarruerNum"         => '',
            "TaxType"            => '',
            "SalesAmount"        => '',
            "InvoiceRemark"      => '',
    ...
    public function Check_Out()
    {
        ...
        $arErrors            = '';
        $aSend_Info          = []; // 送出參數
        $aSend_CheckMac_Info = []; // 送出檢查碼用陣列

        $aReturn_Info          = []; // 回傳參數
        $aReturn_CheckMacValue = []; // 回傳檢查碼用陣列
        ...
        if (sizeof($arErrors) > 0) {
            ...
            // A.一般開立發票
            if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE) {
                // 3-1過濾不需要的項目
            ...
            // B.延遲或觸發開立發票
            if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY) {
                // 3-1過濾不需要的項目
            ...
            // C.開立折讓
            if ($this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE) {
                // 3-1過濾不需要的項目
            ...
            // D.發票作廢
            if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_VOID) {
                // 3-1過濾不需要的項目
                unset($aSend_Info['RelateNumber']);
                unset($aSend_Info['CustomerID']);
                unset($aSend_Info['CustomerIdentifier']);
            ...
            // E.折讓作廢
            if ($this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_VOID) {
                // 3-1過濾不需要的項目
                unset($aSend_Info['RelateNumber']);
                unset($aSend_Info['CustomerID']);
                unset($aSend_Info['CustomerIdentifier']);
            ...
            // F.查詢發票 G.查詢作廢發票
            if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_SEARCH || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_VOID_SEARCH) {
                // 3-1過濾不需要的項目
                unset($aSend_Info['CustomerID']);
                unset($aSend_Info['CustomerIdentifier']);
                unset($aSend_Info['CustomerName']);
            ...
            // H.查詢折讓明細 I.查詢折讓作廢明細
            if ($this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_SEARCH || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_VOID_SEARCH) {
                // 3-1過濾不需要的項目
                unset($aSend_Info['RelateNumber']);
                unset($aSend_Info['CustomerID']);
                unset($aSend_Info['CustomerIdentifier']);
            ...
            // J.發送通知
            if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_NOTIFY) {
                // 3-1過濾不需要的項目
                unset($aSend_Info['RelateNumber']);
                unset($aSend_Info['CustomerID']);
                unset($aSend_Info['CustomerIdentifier']);
            ...
            // K.查詢折讓明細
            if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_TRIGGER) {
                // 3-1過濾不需要的項目
                unset($aSend_Info['RelateNumber']);
                unset($aSend_Info['CustomerID']);
                unset($aSend_Info['CustomerIdentifier']);
            ...
            // L.手機條碼驗證
            if ($this->Invoice_Method == Enum\InvoiceMethod::CHECK_MOBILE_BARCODE) {
                // 3-1過濾不需要的項目
                unset($aSend_Info['RelateNumber']);
                unset($aSend_Info['CustomerID']);
                unset($aSend_Info['CustomerIdentifier']);
            ...
            // M.愛心碼驗證
            if ($this->Invoice_Method == Enum\InvoiceMethod::CHECK_LOVE_CODE) {
                // 3-1過濾不需要的項目
                unset($aSend_Info['RelateNumber']);
                unset($aSend_Info['CustomerID']);
                unset($aSend_Info['CustomerIdentifier']);
            ...
            if (true) {
                // A. B. C. D. E.
                if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_VOID || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_VOID) {
                    // 5-1 CheckMac檢查
                ...
                // F.查詢發票
                if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_SEARCH) {
                    // 5-1 CheckMac檢查
                    if (count($aReturn_Info) > 0 && isset($aReturn_Info['CheckMacValue'])) {
                        $aReturn_CheckMacValue = $aReturn_Info;
                        $sReturn_CheckMacValue = $aReturn_Info['CheckMacValue'];
                ...
                // G.查詢作廢發票
                if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_VOID_SEARCH) {
                    // 5-1 CheckMac檢查
                    if (count($aReturn_Info) > 0 && isset($aReturn_Info['CheckMacValue'])) {
                        $aReturn_CheckMacValue = $aReturn_Info;
                        $sReturn_CheckMacValue = $aReturn_Info['CheckMacValue'];
                ...
                // H.查詢折讓明細
                if ($this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_SEARCH) {
                ...
                // I.查詢折讓作廢明細
                if ($this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_VOID_SEARCH) {
                    // 5-1 CheckMac檢查
                    if (count($aReturn_Info) > 0 && isset($aReturn_Info['CheckMacValue'])) {
                        $aReturn_CheckMacValue = $aReturn_Info;
                        $sReturn_CheckMacValue = $aReturn_Info['CheckMacValue'];
    ...
    public function Check_String()
    {
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_SEARCH || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_VOID_SEARCH) {
            // 4.廠商自訂編號
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY) {
            // 5.客戶代號 CustomerID
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY) {
            ...
            if ($this->Send['Print'] == Enum\PrintMark::Yes) {
            ...
            if ($this->Send['TaxType'] == Enum\TaxType::Zero) {
                if (($this->Send['ClearanceMark'] != Enum\ClearanceMark::Yes) && ($this->Send['ClearanceMark'] != Enum\ClearanceMark::No)) {
            ...
            // *列印註記僅能為 0 或 1
            if (($this->Send['Print'] != Enum\PrintMark::Yes) && ($this->Send['Print'] != Enum\PrintMark::No)) {
                array_push($arErrors, "12:Invalid Print.");
            }
            // *若捐贈註記 = '1' (捐贈)時，則VAL = '0' (不列印)
            if ($this->Send['Donation'] == Enum\Donation::Yes) {
                if ($this->Send['Print'] != Enum\PrintMark::No) {
            ...
            if (($this->Send['Donation'] != Enum\Donation::Yes) && ($this->Send['Donation'] != Enum\Donation::No)) {
                array_push($arErrors, "13:Invalid Donation.");
            }
            // *若統一編號有值時，則VAL = '2' (不捐贈)
            if (strlen($this->Send['CustomerIdentifier']) > 0 && $this->Send['Donation'] == Enum\Donation::Yes) {
            ...
            if (($this->Send['CarruerType'] != Enum\CarruerType::None) && ($this->Send['CarruerType'] != Enum\CarruerType::Member) && ($this->Send['CarruerType'] != Enum\CarruerType::Citizen) && ($this->Send['CarruerType'] != Enum\CarruerType::Cellphone)) {
                array_push($arErrors, "15:Invalid CarruerType.");
            } else {
                // *統一編號不為空字串時，則載具類別不可為會載具或自然人憑證載具
                if (strlen($this->Send['CustomerIdentifier']) > 0) {
                    if ($this->Send['CarruerType'] == Enum\CarruerType::Member || $this->Send['CarruerType'] == Enum\CarruerType::Citizen) {
                        array_push($arErrors, "15:Invalid CarruerType.");
                    }
                }
            }
            ...
            switch ($this->Send['CarruerType']) {
                ...
                case Enum\CarruerType::None:
                case Enum\CarruerType::Member:
                ...
                case Enum\CarruerType::Cellphone:
            ...
            if (($this->Send['TaxType'] != Enum\TaxType::Dutiable) && ($this->Send['TaxType'] != Enum\TaxType::Zero) && ($this->Send['TaxType'] != Enum\TaxType::Free) && ($this->Send['TaxType'] != Enum\TaxType::Mix)) {
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE) {
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY) {
            ...
            if (($this->Send['InvType'] != Enum\InvType::General) && ($this->Send['InvType'] != Enum\InvType::Special)) {
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE) {
            ...
            if (!empty($this->Send['vat'])) {
                if (($this->Send['vat'] != Enum\VatType::Yes) && ($this->Send['vat'] != Enum\VatType::No)) {
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY) {
            // 30.延遲註記 DelayFlag
            if (($this->Send['DelayFlag'] != Enum\DelayFlagType::Delay) && ($this->Send['DelayFlag'] != Enum\DelayFlagType::Trigger)) {
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_TRIGGER) {
            // 33.交易單號 Tsr
            ...
            if ($this->Send['PayType'] != Enum\PayTypeCategory::Ecpay) {
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_DELAY) {
            ...
            switch ($this->Send['PayType']) {
                ...
                case Enum\PayTypeCategory::Ecbank:
                    $this->Send['PayAct'] = 'ECBANK';
                    break;

                // *若交易類別='2'時，則VAL = 'ECPAY'
                case Enum\PayTypeCategory::Ecpay:
                    $this->Send['PayAct'] = 'ECPAY';
                    break;

                // *若交易類別='3'時，則VAL = 'ALLPAY'
                case Enum\PayTypeCategory::Allpay:
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_VOID || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_SEARCH || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_VOID_SEARCH || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_NOTIFY) {
            // 37.發票號碼 InvoiceNo
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE) {
            ...
            // *固定給定下述預設值
            if (($this->Send['AllowanceNotify'] != Enum\AllowanceNotifyType::Sms) && ($this->Send['AllowanceNotify'] != Enum\AllowanceNotifyType::Email) && ($this->Send['AllowanceNotify'] != Enum\AllowanceNotifyType::All) && ($this->Send['AllowanceNotify'] != Enum\AllowanceNotifyType::None)) {
            ...
            if ($this->Send['AllowanceNotify'] == Enum\AllowanceNotifyType::Email && strlen($this->Send['NotifyMail']) == 0) {
            ...
            if ($this->Send['AllowanceNotify'] == Enum\AllowanceNotifyType::Sms && strlen($this->Send['NotifyPhone']) == 0) {
                array_push($arErrors, "40:NotifyPhone is required.");
            }

            // 39-40 通知電子信箱、通知手機號碼不能全為空值 (如果狀態為SMS 或 EMAIL)
            if (strlen($this->Send['NotifyPhone']) == 0 && strlen($this->Send['NotifyMail']) == 0 && ($this->Send['AllowanceNotify'] == Enum\AllowanceNotifyType::Sms || $this->Send['AllowanceNotify'] == Enum\AllowanceNotifyType::Email)) {
                array_push($arErrors, "39-40:NotifyMail or NotifyPhone is required.");
            } else {
                // *下述情況通知手機號碼與電子信箱不可為空值(通知類別為A-皆通知)
                if ($this->Send['AllowanceNotify'] == Enum\AllowanceNotifyType::All && (strlen($this->Send['NotifyMail']) == 0 || strlen($this->Send['NotifyPhone']) == 0)) {
                    array_push($arErrors, "39-40:NotifyMail And NotifyPhone is required.");
                }

                // *下述情況通知手機號碼與電子信箱為空值(通知類別為N-皆不通知)
                if ($this->Send['AllowanceNotify'] == Enum\AllowanceNotifyType::None && (strlen($this->Send['NotifyMail']) > 0 || strlen($this->Send['NotifyPhone']) > 0)) {
        ...
        // 檢查 D.發票作廢、折讓作廢
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_VOID || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_VOID) {
            // 43.作廢原因 Reason
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_VOID || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_SEARCH || $this->Invoice_Method == Enum\InvoiceMethod::ALLOWANCE_VOID_SEARCH || $this->Invoice_Method == Enum\InvoiceMethod::INVOICE_NOTIFY) {
            // 44.折讓編號 AllowanceNo
        ...
        if ($this->Invoice_Method == Enum\InvoiceMethod::INVOICE_NOTIFY) {
            ...
            if ($this->Send['Notify'] == Enum\NotifyType::Email && strlen($this->Send['NotifyMail']) == 0) {
                array_push($arErrors, "39:NotifyMail is required.");
            }
            // *UrlEncode
            $this->Send['NotifyMail'] = urlencode($this->Send['NotifyMail']);
            ...
            if ($this->Send['Notify'] == Enum\NotifyType::Sms && strlen($this->Send['Phone']) == 0) {
            ...
            if (strlen($this->Send['Phone']) == 0 && strlen($this->Send['NotifyMail']) == 0) {
                ...
                if ($this->Send['Notify'] == Enum\NotifyType::All && (strlen($this->Send['NotifyMail']) == 0 || strlen($this->Send['Phone']) == 0)) {
            ...
            // *固定給定下述預設值
            if (($this->Send['Notify'] != Enum\NotifyType::Sms) && ($this->Send['Notify'] != Enum\NotifyType::Email) && ($this->Send['Notify'] != Enum\NotifyType::All)) {
            ...
            // *固定給定下述預設值
            if (($this->Send['InvoiceTag'] != Enum\InvoiceTagType::Invoice) && ($this->Send['InvoiceTag'] != Enum\InvoiceTagType::Invoice_Void) && ($this->Send['InvoiceTag'] != Enum\InvoiceTagType::Allowance) && ($this->Send['InvoiceTag'] != Enum\InvoiceTagType::Allowance_Void) && ($this->Send['InvoiceTag'] != Enum\InvoiceTagType::Invoice_Winning)) {
            ...
            // *固定給定下述預設值
            if (($this->Send['Notified'] != Enum\NotifiedType::Customer) && ($this->Send['Notified'] != Enum\NotifiedType::vendor) && ($this->Send['Notified'] != Enum\NotifiedType::All)) {
